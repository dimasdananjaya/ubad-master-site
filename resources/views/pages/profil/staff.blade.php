@extends('layouts.app')

@section('content')
<section id="staff" class="pb-5">
    <div class="container">
        <h1 class="text-center">Dosen Universitas Bali Dwipa</h1>
        <div class="row">
            <!-- Team member -->
                <div class="card-deck">
                    <div class="row">

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                            <p class="text-center btn btn-primary rektor mx-auto d-block"><b>REKTOR</b></p>
                            <img class="card-img-top mx-auto" src="/resources/dosen/ketutsuriasih.jpg" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">Dr. Ir. Ketut Suriasih, M.App.Sc</h5>
                                    <p class="small-center"><small>Dosen Prodi Teknologi Pangan</small></p>
                                    <hr>
                                    <div class="d-flex justify-content-center">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s3.png" alt="Card image cap">
                                    </div>
                                    <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s2">S2 : University of New South Wales</p></div>
                                    <div class="col-lg-12"><p class="card-text s3">S3 : Universitas Udayana</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                            <p class="text-center btn btn-primary rektor mx-auto d-block"><b>Ketua Yayasan</b></p>
                            <img class="card-img-top mx-auto" src="/resources/dosen/wayanadnyana.jpg" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">Dr. Ir. I Wayan Adnyana S.H, M.Kn</h5>
                                    <p class="small-center"><small>Dosen Prodi Hukum</small></p>
                                    <hr>
                                    <div class="d-flex justify-content-center">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s3.png" alt="Card image cap">
                                    </div>
                                    <div class="col-lg-12"><p class="card-text s1">S1 : Institut Teknologi Bandung, Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s2">S2 : WCIMT, Universitas Airlangga</p></div>
                                    <div class="col-lg-12"><p class="card-text s3">S3 : Universitas Brawijaya</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                            <img class="card-img-top mx-auto" src="/resources/dosen/profsucipta.jpeg" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">Prof. Ir. I Nyoman Sucipta M.P</h5>
                                    <p class="small-center"><small>Dosen Prodi Teknologi Pangan</small></p>
                                    <hr>
                                    <div class="d-flex justify-content-center">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s3.png" alt="Card image cap">
                                    </div>
                                    <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s2">S2 : Universitas Gadjah Mada</p></div>
                                    <div class="col-lg-12"><p class="card-text s3">S3 : Universitas Udayana</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                            <img class="card-img-top mx-auto" src="/resources/dosen/ketutsuardana.jpg" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">Dr. I Ketut Suardana S.S, M.Hum</h5>
                                    <p class="small-center"><small>Dosen Prodi Bahasa Inggris</small></p>
                                    <hr>
                                    <div class="d-flex justify-content-center">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s3.png" alt="Card image cap">
                                    </div>
                                    <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s2">S2 : Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s3">S3 : Universitas Udayana</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                            <img class="card-img-top mx-auto" src="/resources/dosen/puja.jpg" alt="Card image cap">
                                <div class="card-body">
                                    <h5 class="card-title">Ida Bagus Puja Arimbawa S.Si, M.Si</h5>
                                    <p class="small-center"><small>Dosen Prodi Sistem Informasi</small></p>
                                    <hr>
                                    <div class="d-flex justify-content-center">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                        <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                    </div>
                                    <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                    <div class="col-lg-12"><p class="card-text s2">S2 : Institut Teknologi Bandung</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                                <img class="card-img-top mx-auto" src="/resources/dosen/madejuliarta.jpeg" alt="Card image cap">
                                <div class="card-body">
                                <h5 class="card-title">I Made Juliarta S.S, M.Hum</h5>
                                <p class="small-center"><small>Dosen Prodi Bahasa Inggris</small></p>
                                <hr>
                                <div class="d-flex justify-content-center">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                </div>
                                <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                <div class="col-lg-12"><p class="card-text s2">S2 : Universitas Udayana</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                                <img class="card-img-top mx-auto" src="/resources/dosen/ekasuardana.jpg" alt="Card image cap">
                                <div class="card-body">
                                <h5 class="card-title">I Putu Eka Suardana S.S, M.Hum</h5>
                                <p class="small-center"><small>Dosen Prodi Bahasa Inggris</small></p>
                                <hr>
                                <div class="d-flex justify-content-center">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                </div>
                                <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                <div class="col-lg-12"><p class="card-text s2">S2 : Universitas Udayana</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->
                        
                        <div class="col-lg-4 col-md-6 col-xs-12">
                            <div class="card">
                                <img class="card-img-top mx-auto" src="/resources/dosen/sriwiraswati.jpeg" alt="Card image cap">
                                <div class="card-body">
                                <h5 class="card-title">Anak Agung Sri Wiraswati S.Psi, M.Psi, Psikolog</h5>
                                <p class="small-center"><small>Dosen Prodi Psikologi</small></p>
                                <hr>
                                <div class="d-flex justify-content-center">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s1.png" alt="Card image cap">
                                    <img class="logo-pendidikan mx-auto" src="/resources/logo/s2.png" alt="Card image cap">
                                </div>
                                <div class="col-lg-12"><p class="card-text s1">S1 : Universitas Udayana</p></div>
                                <div class="col-lg-12"><p class="card-text s2">S2 : Universitas Surabaya</p></div>
                                </div>
                            </div><!--endcard-->
                        </div><!-- end col-->

                    </div><!-- end row-->
                </div><!--card deck-->
            <!-- ./Team member -->      
        </div><!--row-->
        <h1 class="text-center" style="margin-top: 30px;">Dan Lebih Dari 20+ Dosen Lainnya </h1>
        <p class="text-center" style="color: #fff;"><small>Halaman Ini Akan Diperbarui Untuk <br> Menampilkan Seluruh Dosen</small></p>
</section>

@endsection